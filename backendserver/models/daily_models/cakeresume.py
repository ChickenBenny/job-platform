...
class web_crawler_cake():
    ...
    def find_next_page(self, soup):
        next_page = soup.find('div', {'class' : re.compile(r'Pagination_wrapper__*')})
        if not next_page:
            return ""
        next_page_link = next_page.find_all('a', {'class': re.compile(r'Pagination_itemNavigation__*')}, href = True)
    ...
    def get_search_item(self, soup):
        search_item = soup.find_all('div', {'class': re.compile(r'JobSearchItem_wrapper__*')})
        return search_item

    def get_job_link_and_title(self, search_item):
        url_domain = 'https://www.cakeresume.com'
        job_title = search_item.find('div', {'class': re.compile(r'JobSearchItem_headerTitle__*')}).getText()
        job_link = url_domain + (search_item.find('div', {'class': re.compile(r'JobSearchItem_headerTitle__*')}).find('a', href = True))['href']
        return job_link, job_title

    def get_company_name(self, search_item):
        company_name = search_item.find('div', class_ = re.compile(r'JobSearchItem_headerSubtitle__*')).getText()
        return company_name

    def get_job_skill(self, search_item):
        skill_set = []
        if search_item.find('div', {'class': re.compile(r'JobSearchItem_tags__*')}):
            tags = search_item.find('div', {'class': re.compile(r'JobSearchItem_tags__*')}).find_all('div', {'class': re.compile(r'Tags_item__*')})
            for tag in tags:
                if tag.text != '':
    ...
    def get_feature(self, search_item):
        ...
        inline_messages_type = job_features.find_all('div', class_ = re.compile(r'InlineMessage_icon__*'))
        inline_messages = job_features.find_all('div', class_ = re.compile(r'InlineMessage_label__*'))     
        feature = {}
    ...
    def get_job_data(self, search_item):
        ...
        try:
            data = [job_link, job_title, company_name, skill_set, feature['user'], feature['map-marker-alt'], feature['dollar-sign']]
            return data
        except:
            return []
